...
Hooks.once("init", async function() {
    ...
    CONFIG.Token.defaults = CONFIG.Token.defaults || {};
    CONFIG.Token.defaults.vision = true;
    CONFIG.Token.defaults.bar1 = { attribute: "system.wounds", max: "max", value: "value" };
    CONFIG.Token.defaults.bar2 = { attribute: "system.fate", max: "max", value: "value" };
...
Hooks.on("renderChatMessage", (message, html, data) => {
    ...
    if (rollData?.flags?.isDamageRoll) {
        ...
        if (canRevert && revertContainer.length) {
            // Show button if user has permission and damage was applied
            revertContainer.show();
        } else if (revertContainer.length) {
            // Hide button if user doesn't have permission
            revertContainer.hide();
        }
        
        // Always attach click handler if button exists
        html.find(".manual-damage-undo").off("click").on("click", onManualDamageUndo);
    }
});
